...
function FoodList(){
    ...
    let row=[]
    if(startPage>1){
        row.push(<li><a onClick={prev}>&lt;</a></li>)
    }
    for(let i=startPage;i<=endPage;i++){
        if(i==curpage){
            row.push(<li className={"active"}><a onClick={()=>pageChagne(i)}>{i}</a></li>)
        }else{
            row.push(<li><a onClick={()=>pageChagne(i)}>{i}</a></li>)
        }
    }
    if(endPage<totalpage){
        row.push(<li><a onClick={next}>&gt;</a></li>)
    }
